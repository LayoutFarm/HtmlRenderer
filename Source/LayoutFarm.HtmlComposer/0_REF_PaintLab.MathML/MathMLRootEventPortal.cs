...
using PaintLab.MathML;

namespace PaintLab.Svg
{
    partial class MathMLRootEventPortal
    {
        readonly Stack<MathMLHitChain> _hitChainPools = new Stack<MathMLHitChain>();
        readonly HtmlElement _elementNode;
        public MathMLRootEventPortal(HtmlElement elementNode)
        {
            _elementNode = elementNode;
        }
        public CssBoxSvgRoot SvgRoot { get; set; }

        MathMLHitChain GetFreeHitChain() => (_hitChainPools.Count > 0) ? _hitChainPools.Pop() : new MathMLHitChain();

        void ReleaseHitChain(MathMLHitChain hitChain)
        {
            hitChain.Clear();
